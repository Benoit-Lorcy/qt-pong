...
Player::Player(PlayerSide initside, QGraphicsItem *parent) : QGraphicsRectItem(parent), side(initside)
{
    goUp = false;
    goDown = false;
...
void Player::reset(int boardWidth, int boardHeight)
{
    ...
    {
        this->setRect(-(boardWidth / 2) - (paddlew / 2) + (paddleDistance), -paddleh / 2, paddlew, paddleh);
    }
...
void Player::paddlesSettings(qreal paddlew, qreal paddleh, qreal distance, qreal vitesse)
{
