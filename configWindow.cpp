...
QGroupBox *ConfigWindow::createBalleGroup()
{
    ...
    addCustomSlider("Acceleration", grid, BALL_INCREMENT_SPEED, 0.01, 1, ballList);
...
QGroupBox *ConfigWindow::createPaddlesGroup()
{
    ...
    addCustomSlider("Distance", grid, PADDLE_DISTANCE, 1, 50, paddlesList);
    addCustomSlider("Vitesse", grid, PADDLE_SPEED, 1, 40, paddlesList);
    groupBox->setLayout(grid);
...
QGroupBox *ConfigWindow::createSceneGroup()
{
    ...
    addCustomSlider("Taille texte", grid, TEXT_SIZE, 1, 8, sceneList);
...
{
    emit ballSettings(ballList[0]->value(), ballList[1]->value(), ballList[2]->value(), ballList[3]->value());
    emit paddlesSettings(paddlesList[0]->value(), paddlesList[0]->value(), paddlesList[0]->value(), paddlesList[0]->value());
    emit controlesSettings(sceneList[0]->value(), sceneList[0]->value());
    //emit SceneSettings();
}

void ConfigWindow::addCustomSlider(QString name, QGridLayout *grid, int current, int min, int max, QVector<QSlider *> &list)
{
